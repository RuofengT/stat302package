...
Based on CV misclassification rate, I would choose k = 1 as well, CV misclassfication rate lowest = 0.175, 1-nearest-neighbor.
...
g2 <- ggplot() + 
  ...
  scale_x_discrete( ) +
  labs(x = "k = 2", y = "MSE Errors")

# plot when k = 5
g5 <- ggplot() + 
  geom_boxplot(aes(y = results_rf[, 2])) + 
  scale_x_discrete( ) +
  labs(x = "k = 5", y = "MSE Errors")

# plot when k = 10
g10 <- ggplot() + 
  geom_boxplot(aes(y = results_rf[, 3])) + 
  scale_x_discrete( ) +
  labs(x = "k = 10", y = "MSE Errors")

# arrange 3 boxplots together
grid.arrange(g2, g5, g10, nrow = 1, top = "MSE Errors for k = 2, 5, 10")
```
